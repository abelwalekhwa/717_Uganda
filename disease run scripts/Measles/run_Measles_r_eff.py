...
to_run = [{"initial": 3, # number of seed infections
...
def run_scenario(kwargs, filter=False):
    ...
    response_time = 0 * np.ones(nruns)
    daily_vaccs = 0 * np.ones(nruns)
    baseline_vax_coverages = 0 * np.ones(nruns)

    seeds, betas = get_betas(mean_beta, std_beta, len(response_time))
    if not hasattr(thread_local, "pbar"):
        thread_local.pbar = tqdm(total=len(seeds))
    pbar = thread_local.pbar
    description = "baseline_lattice"
    ...
    tasks = []
    for seed in seeds:
        kwargs.update({"baseline_vax_coverage": baseline_vax_coverages[seed]})
        tasks.append(run_calibration.s(seed, betas[seed], response_time[seed], daily_vaccs[seed], disease='measles',  **kwargs))
    ...
    if result.successful():
        ...
        if filter:
            ...
            Samples.new(result_dir, outputs, ["seed"], fname= "r_eff_beta_" + str(mean_beta) + "_calibration.zip")
    else:

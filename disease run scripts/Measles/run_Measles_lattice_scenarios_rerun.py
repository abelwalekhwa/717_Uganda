...
def run_scenario(kwargs, filter=False):
    ...
    description_baseline = "Baseline_Beta_" + str(mean_beta) + "_ResponseTime" + str(response_time) + '_DailyVaxRate' + \
                  str(daily_vacc) + '_BaselineCoverage' + str(kwargs["baseline_vax_coverage"])

    # Read in seeds and betas
    with zipfile.ZipFile("results/Baseline/" + description_baseline + ".zip") as z:
        with z.open("summary.csv") as summary_file:
            summary_file_df = pd.read_csv(summary_file)
            seeds = summary_file_df["seed"]
            betas = summary_file_df["beta"]

    description = "Scenarios_Beta_" + str(mean_beta) + "_ResponseTime" + str(response_time) + '_DailyVaxRate' + \
                  str(daily_vacc) + '_BaselineCoverage' + str(kwargs["baseline_vax_coverage"])
    ...
    if kwargs["name"] == "NoORI":
        response_time_adjusted = 0
        daily_vacc_adjusted = 0
        fname = description + "_NoORI"
    if kwargs["name"] == "7day":
        response_time_adjusted = 7
        daily_vacc_adjusted = kwargs["daily_vacc"]
        fname = description + "_7day"
    ...
    tasks = []
    for seed in seeds:
            tasks.append(run_calibration.s(seed, betas[seed], response_time=response_time_adjusted,
                                           daily_vacc=daily_vacc_adjusted, disease='measles',  **kwargs))
...
if __name__ == "__main__":
    ...
    to_run.append({"name": "NoORI", "initial": 3,  # number of seed infections
                            ...
                            "popsize": 50000,
                            "daily_vacc": 500 * (1-80/100),
                            "response_time": 170,
                            "baseline_vax_coverage": 80,
                            "filter": False})

    to_run.append({"name": "NoORI", "initial": 3,  # number of seed infections
                            "test_prob": 0.10,  # testing prob per day
                            "popsize": 50000,
                            "daily_vacc": 7000 * (1-80/100),
                            "response_time": 220,
                            "baseline_vax_coverage": 80,
                            "filter": False})

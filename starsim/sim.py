...
class Sim(sc.prettyobj):
    ...
    def init_interventions(self):
        ...
        for i, intervention in enumerate(self.pars['interventions']):
            ...
            if isinstance(intervention, ss.Intervention):
                ...
                if intervention.label is not None:
                    self.interventions.append(intervention, key=intervention.label)
                else:
    ...
    def get_intervention(self, label=None, partial=False, first=False, die=True):
        ...
        '''
        return self._get_interventions(label=label, partial=partial, first=first, die=die, as_inds=False, as_list=False)
    ...
    def run(self, until=None, reset_seed=True, verbose=None):
        ...
        while self.ti < until:
            ...
            if self.pars['stopping_func'] and self.pars['stopping_func'](self):
    ...
    def finalize(self, verbose=None):
        ...
        if self.diseases[0].name == 'measles' or self.diseases[0].name == 'yellow_fever':
            self.compute_results(verbose=verbose) # Calculate the rest of the results, e.g. compute R_eff
    ...
    def _get_interventions(self, label=None, partial=False, as_list=False, as_inds=False, die=True, first=False):
        ''' Helper method for get_interventions() '''

        # Handle inputs

        ia_list = sc.tolist(self.pars['interventions']) # List of interventions
        n_ia = len(ia_list) # Number of interventions
        ...
        if label == 'summary': # Print a summary of the interventions
            ...
            print(f'Summary of interventions:')
            print(df)
            return
            ...
            for label in labels:
                if sc.isnumber(label):
                    ...
                    errormsg = f'Could not interpret label type "{type(label)}": should be str, int, list, or intervention class'
                    raise TypeError(errormsg)
            ...
            if as_inds:
                ...
                if len(matches) == 0: # pragma: no cover
                    if die:
                        errormsg = f'No interventions matching "{label}" were found'
                        raise ValueError(errormsg)
                    else:
                        output = None
                else:
    ...
    def compute_r_eff(self, smoothing=2, disease='yellow_fever'):
        disease = self.diseases[0].name
        # Find the dates that everyone became infectious and recovered, and hence calculate infectious duration
        recov_inds = self.diseases[0].recovered
        dead_inds = self.diseases[0].dead
        date_recov = self.diseases[0].ti_recovered[recov_inds]
        date_dead = self.diseases[0].ti_dead[dead_inds]
        date_outcome = np.concatenate((date_recov, date_dead))
        # inds = np.concatenate((recov_inds, dead_inds))
        date_inf = self.diseases[0].ti_infectious[recov_inds or dead_inds]
        if len(date_outcome):
        ...
        new_infections = self.results[disease + '-new_infections'].values # - self.results['n_imports'].values
        n_infectious = self.results[disease + '-n_infectious'].values
        ...
        # dur_pars = self.results["measles-outbreak_dur"][0]

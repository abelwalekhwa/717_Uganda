...
>>> dist = starsim.normal(1,1) # Make a distribution
>>> dist()  # Draw a sample
>>> dist(10) # Draw several samples
>>> dist.sample(10) # Same as above
>>> starsim.State('foo', float, fill_value=dist)  # Use distribution as the fill value for a state
...
>>> starsim.poisson(rate=1).sample(n=10)  # Sample from a temporary distribution
"""
...
__all__ = [
    'Distribution', 'uniform', 'uniform_discrete', 'choice', 'normal', 'normal_pos', 'normal_int', 'normal_pos_int', 'lognormal', 'lognormal_int',
...
class uniform_discrete(Distribution):
    """
    ...
    def sample(self, n=1):
        return np.random.random_integers(low=self.low, high=self.high, size=n)
...
class normal_pos_int(normal_pos):
    """
    Right sided normal returning only integer values
    """
